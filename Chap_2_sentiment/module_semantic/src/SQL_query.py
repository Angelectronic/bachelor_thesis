...
class sql_operation:
    ...
    def get_info(self, table, distinct_row = "", condition = None, batch_size = 1000):
        if condition == None:
            if distinct_row != "":
        ...
        while True:
            batch_query = f"{select_query} ORDER BY (SELECT NULL) OFFSET 0 ROWS FETCH NEXT {batch_size} ROWS ONLY"
            print(batch_query)
            try:
